...
const { incr, DB_SLOW_THRESHOLD_MS } = require('./metrics');
...
try { db.exec('ALTER TABLE users ADD COLUMN password_changed_at TEXT DEFAULT (datetime(\'now\'))'); } catch {}
try { db.exec('ALTER TABLE users ADD COLUMN failed_login_attempts INTEGER DEFAULT 0'); } catch {}
try { db.exec('ALTER TABLE users ADD COLUMN locked_until TEXT DEFAULT NULL'); } catch {}

// Instrumented statement wrapper - tracks query timing
function timedStmt(rawStmt, name) {
  return {
    get(...args) {
...
function trackQuery(name, startTime) {
...
const stmt = {
  ...
  // Account lockout
  incrementFailedLogins: timedStmt(db.prepare('UPDATE users SET failed_login_attempts = failed_login_attempts + 1 WHERE id = ?'), 'incrementFailedLogins'),
  ...
  purgeDelivered: timedStmt(db.prepare("DELETE FROM messages WHERE delivered = 1 AND timestamp < datetime('now', '-24 hours')"), 'purgeDelivered'),
  purgeStale: timedStmt(db.prepare("DELETE FROM messages WHERE delivered = 0 AND timestamp < datetime('now', '-30 days')"), 'purgeStale'),
...
function audit(event, { userId, username, ip, details } = {}) {
  try {
...
const getPreKeyBundle = db.transaction((userId) => {
  const identity = stmt.getIdentityKey.get(userId);
  if (!identity) return null;

  const signedPreKey = stmt.getSignedPreKey.get(userId);
  if (!signedPreKey) return null;

  const oneTimePreKey = stmt.consumeOneTimePreKey.get(userId);
...
const uploadBundle = db.transaction((userId, bundle) => {
...
function getConversationPartners(userId) {
  return stmt.getConversationPartners.all(userId, userId).map(r => r.partner_id);
}

module.exports = { db, stmt, getPreKeyBundle, uploadBundle, deleteUser, audit, getConversationPartners };
